...
Class App {
    ...
    hidden [string] GetHostPath([string] $out) {
        ...
        while (-not (Test-Path -LiteralPath $hostPath -PathType Container)) {
            ...
            $hostPath = [string]::IsNullOrWhiteSpace($hostPath) ? $PSScriptRoot : $hostPath
        }
    ...
    hidden [void] GenerateHost([string] $hostPath) {
        ...
        foreach ($listRule in [Encoding]::UTF8.GetString([Convert]::FromBase64String((Invoke-WebRequest "https://gitlab.com/gfwlist/gfwlist/raw/master/gfwlist.txt"))) -split "`n") {
            if (-not $isGeneralList) {
                if ($listRule.Contains("General List Start")) {
                    Set-Content -LiteralPath (Join-Path $hostPath "Cealing-Host-List.json") "["
            ...
            elseif ($listRule.Contains("General List End")) {
                Add-Content -LiteralPath (Join-Path $hostPath "Cealing-Host-List.json") "]" -NoNewline

                return
            }

            if ($listRule -match "^[!@\[/]" -or [string]::IsNullOrWhiteSpace($listRule)) { continue }

            [string] $listDomain = $listRule.Trim() -replace "^[|.*]+" -replace "^https?://" -replace "/.*$"
            [int] $tryCount = 3

            if ($listDomain -eq $lastListDomain) { continue }
            $lastListDomain = $listDomain

            while ($true) {
                try {
                    [array] $hostIpAnswer = (Invoke-RestMethod "https://ns.net.kg/dns-query?name=$listDomain").Answer

                    if ($hostIpAnswer) { Add-Content -LiteralPath (Join-Path $hostPath "Cealing-Host-List.json") "`t[[""*$listDomain""],"""",""$($hostIpAnswer[-1].data)""]," }
